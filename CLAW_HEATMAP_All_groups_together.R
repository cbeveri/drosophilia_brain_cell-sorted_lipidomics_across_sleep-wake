...
# names_to_include <- read.csv("trem_2_all_unfiltered_Matched_S3_theirs_5.csv")
# names_to_include <- names_to_include$lipid
...
create_heatmap_Z_score <- function(df1, df2, title,names_to_include) {
  ...
  columns_to_save <- c("lipid", intensity_columns)
  
  # Subset the dataframe to include only these columns
  subset_df <- df[columns_to_save]
  subset_df <- subset_df %>% filter(lipid %in% names_to_include$lipid)
  
  # Write the subsetted dataframe to a CSV file
  write.csv(subset_df, "Blank_subtracted.csv", row.names = FALSE)
  ...
  orange <- rgb(239/255, 151/255, 8/255)  # Converts RGB to hexadecimal for orange
  cyan <- rgb(15/255, 207/255, 192/255)   # Converts RGB to hexadecimal for cyan
  white <- rgb(1, 1, 1)                   # White color
  
  # Create the color palette function including white as the midpoint
  color_palette <- colorRampPalette(c(cyan, white, orange))
...
create_heatmap_Z_score_reorder <- function(df1, df2, title) {
  ...
  annotation_labels <- c(rep(labels_title_2, each=df$Length2[1]), 
                         rep(labels_title_1, each=df$Length1[1]),
                         rep(labels_title_4, each=df$Length2_df2[1]),
                         rep(labels_title_3, each=df$Length1_df2[1]))
  ...
  # Define the colors in hexadecimal format
  orange <- rgb(239/255, 151/255, 8/255)   # Converts RGB to hexadecimal
  cyan <- rgb(15/255, 207/255, 192/255)    # Converts RGB to hexadecimal
  ...
  # Save the heatmap
  save_pheatmap_pdf(heatmap_obj, paste0("heatmaps_large_reordered/", title, "_Zscore_2_limit.pdf"), width = 11, height = 10)
}
